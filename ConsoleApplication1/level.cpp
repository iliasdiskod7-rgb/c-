...
void Level::init() {
    ...
    for (int i = 0; i < 10; i++) {
        for (int j = 0; j < 10; j++) {
            ...
            if (rand() % 5 == 0) c->m_is_obstacle = true;
...
void Level::update(float dt) {
    ...
    mouse_box.m_width = 0.1f;
    mouse_box.m_height = 0.1f;

    // 3. Έλεγχος για κλικ 
    if (ms.button_left_pressed) {
        for (auto cell : m_cells) {
            // Αν το ποντίκι "ακουμπάει" το κελί
            if (cell->intersect(mouse_box)) {
                cell->m_is_obstacle = !cell->m_is_obstacle; // Toggle εμποδίου
